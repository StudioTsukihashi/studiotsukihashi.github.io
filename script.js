...
// Version 3.6 – gallery filters and subtle hero parallax
document.querySelectorAll(".gallery-filter-button").forEach((button) => {
  button.addEventListener("click", () => {
    document.querySelectorAll(".gallery-filter-button").forEach((item) => item.classList.remove("is-active"));
...
if (cinematicHero && !window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
  window.addEventListener("scroll", () => {
    const offset = Math.min(window.scrollY * 0.08, 45);
    cinematicHero.style.setProperty("--hero-shift", `${offset}px`);
    cinematicHero.style.backgroundPosition = `center ${offset}px`;
  }, { passive: true });
}
